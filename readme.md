...
- 演示页面：提供了基础参数控制面板，您可以在控制面板中调整参数，实时查看效果。选择预设图片和上传自定义图片查看不同图像的显示效果。

- 在项目中使用：将./src/dpis.js 复制到您的项目中，并引入即可使用。
您主要需要关注以下几个方法：
    - 使用 `dpis=new DPIS(canvasElementId, dpisAttrs)` 创建dpis实例，并初始化画布和参数
    - `dpis.updateConfig(dpisAttrs)`：更新参数配置
    - `dpis.loadAndBuildImage(image_url)`：传入图片URL并构建粒子图形
    - `dpis.renew()`：重置画布、重载图片，重启动画
